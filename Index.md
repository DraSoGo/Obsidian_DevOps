# 📘 DevOps Lecture Index

> [!summary]
> Topic index for the entire DevOps study — each topic is a major folder that gets filled with notes as the lectures progress.

## 🗂️ Main-Topic

### 🧱 Foundation
- **[[Basic_knowledge/linux|Basic Knowledge]]** — Linux, Network, Applications, Home Server Runbook
- **Linux** — *coming soon*
- **Shell Script** — *coming soon*
- **Git** — *coming soon*

### 📦 Container & Orchestration
- **Docker** — *coming soon*
- **Kubernetes** — *coming soon*
- **Helm** — *coming soon*
- **Istio** — *coming soon*

### ⚙️ CI/CD & Automation
- **Jenkins** — *coming soon*
- **GitOps** — *coming soon*
- **Terraform** — *coming soon*

### 💻 Programming
- **Python** — *coming soon*
- **Golang** — *coming soon*

### 🎓 Career
- **PCA** — *coming soon*
- **Interview** — *coming soon*

---

## 📌 Current Progress

| Topic | Status | Notes |
|---|---|---|
| Basic Knowledge | 🟢 In progress | [[Basic_knowledge/linux|linux]], [[Basic_knowledge/network|network]], [[Basic_knowledge/applications|applications]], [[Basic_knowledge/server_nas_command|server_nas_command]] |
| Linux | ⚪ Not started | — |
| Shell Script | ⚪ Not started | — |
| Git | ⚪ Not started | — |
| Docker | ⚪ Not started | — |
| Kubernetes | ⚪ Not started | — |
| Helm | ⚪ Not started | — |
| Istio | ⚪ Not started | — |
| Jenkins | ⚪ Not started | — |
| GitOps | ⚪ Not started | — |
| Terraform | ⚪ Not started | — |
| Python | ⚪ Not started | — |
| Golang | ⚪ Not started | — |
| PCA | ⚪ Not started | — |
| Interview | ⚪ Not started | — |

---

## 🔄 How to update this page

When a new topic starts:
1. Create the folder in the vault named after the topic
2. Change the `**Name** — *coming soon*` line into a link `**[[Folder/first-page|Name]]**`
3. Update the status in the Progress table (🟢 In progress / 🟡 Draft / ✅ Done)
4. Push to GitHub — the website updates automatically
